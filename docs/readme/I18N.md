...
| ca   | Catalan            | Full support       |
| pl   | Polish             | Full support       |
| tl   | Tagalog (Filipino) | Full support       |
...
src/i18n/
├── __tests__/
│   ├── index.rntl.ts           # i18n initialisation tests
│   └── localesParity.rntl.ts   # Translation completeness tests
├── locales/
...
Edit all locale files (`en.json`, `es.json`, `ca.json`, `pl.json`, `tl.json`). English is the reference language - add keys there first, then translate to other languages:
...
| English (UK)     | English      | Base language match      |
| Spanish (Spain)  | Spanish      | Exact match              |
| Spanish (Mexico) | Spanish      | Base language match      |
| Catalan          | Catalan      | Exact match              |
| Polish           | Polish       | Exact match              |
| Tagalog          | Tagalog      | Exact match              |
...
```typescript
// src/features/Settings/store/reducer.ts
export type Language = 'en' | 'es' | 'ca' | 'pl' | 'tl' | 'fr';
```
...
For more details, see the [i18next docs](https://www.i18next.com/) or [react-i18next docs](https://react.i18next.com/).
