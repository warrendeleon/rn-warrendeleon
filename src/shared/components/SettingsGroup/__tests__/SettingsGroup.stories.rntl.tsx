...
describe('SettingsGroup Stories', () => {
  it('renders SingleItem story with item label', async () => {
  ...
  it('renders TwoItems story with both labels', async () => {
  ...
  it('renders ThreeItems story with all three labels', async () => {
    const { args } = stories.ThreeItems;
    const { getByText } = await renderWithProviders(<SettingsGroup items={args!.items!} />);
    args!.items!.forEach(item => {
      expect(getByText(item.label)).toBeOnTheScreen();
    });
  });

  it('renders WithoutIcons story with labels visible', async () => {
  ...
  it('renders Mixed story with all item labels', async () => {
  ...
  it('ThreeItems story displays all labels', async () => {
    const { args } = stories.ThreeItems;
    const { getByText } = await renderWithProviders(<SettingsGroup items={args!.items!} />);
  ...
  it('WithoutIcons story displays labels without icons', async () => {
    const { args } = stories.WithoutIcons;
    const { getByText } = await renderWithProviders(<SettingsGroup items={args!.items!} />);
    expect(getByText('Privacy Policy')).toBeOnTheScreen();
    expect(getByText('Terms of Service')).toBeOnTheScreen();
