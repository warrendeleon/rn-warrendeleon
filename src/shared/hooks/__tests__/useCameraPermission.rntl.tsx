 ...
 *
 * EAA compliance: Tests permission states for camera access
 */
...
describe('useCameraPermission', () => {
  ...
  describe('checkPermission', () => {
    ...
    it('handles errors and sets UNAVAILABLE', async () => {
      ...
      expect(mockLogError).toHaveBeenCalledWith('Failed to check camera permission', error);
    });
  ...
  describe('requestPermission', () => {
    ...
    it('handles errors gracefully', async () => {
      ...
      expect(mockLogError).toHaveBeenCalledWith('Failed to request camera permission', error);
    });
  ...
  describe('derived state', () => {
    ...
    it('canProceed: true only when isGranted', async () => {
      ...
      await waitFor(
        () => {
          expect(deniedResult.current.isLoading).toBe(false);
        },
        { timeout: 3000, interval: 100 }
      );

      expect(deniedResult.current.canProceed).toBe(false);
    });
  });
