...
Feature: Mock Status Verification
  ...
  Scenario: View mock status screen shows all Portfolio API mocked data
    ...
    And I should see an element with testID "mock-status-profile"
    And I should see an element with testID "mock-status-education"
    And I should see an element with testID "mock-status-work-experience"
    # Check for state-specific testIDs to avoid false positives (Not Mocked contains Mocked)
    And I should see an element with testID "mock-status-profile-mocked"
    And I should see an element with testID "mock-status-education-mocked"
    And I should see an element with testID "mock-status-work-experience-mocked"
  ...
  Scenario: View mock status screen shows Supabase Auth API is mocked
    ...
    And I should see an element with testID "mock-status-auth-api"
    And I should see an element with testID "mock-status-auth-api-mocked"
